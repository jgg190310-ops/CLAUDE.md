...
(function () {
  ...
  function nbAnswer(q) {
    ...
    // emagrecer
    if (/(emagrec|perder peso|perder gordura|secar|déficit|deficit|perder \d|perder uns \d|perder alguns quilos|sem passar fome|perder \d+\s?kg)/.test(norm)) {
    ...
    if (/(metabolismo|metabolismo lento|destravar|emagrecer mais rapido|nao emagreco|não emagreço|travou|plato|platô|estagn|nao perco|não perco|parei de emagrecer|balanca (nao|não) (sai|desce|mexe)|balança (nao|não)|sem resultado|vejo resultado|empac)/.test(norm)) {
    ...
    if (/(compuls|fome emocional|fome (e|é) emocional|belisc|ansiedade de comer|comer demais|descontar na comida|vontade de doce a noite|vontade de doce à noite|ataco a geladeira|ataco a comida|ataco (o |a )?doce|tedio|tédio|despensa|vou direto na (geladeira|cozinha|despensa)|bato muita comida|bater comida|como escondido|comer escondido|comer sozinh|quando (to|tô|fico|estou|fica) (triste|nervos|ansios|estress|chatead)|brig(o|uei|ar) com|fome ou cabeca|fome ou cabeça|fome ou emo|de madrugada|meio da madruga|acordo (pra|para) comer|uso a comida|comida pra me acalmar|comida para me acalmar|so quero comida|só quero comida|desconto na comida|vontade louca de comer)/.test(norm)) {
    ...
    // quanto tempo pra criar hábito / constância
    if (/(virar habito|virar hábito|criar habito|criar hábito|quanto tempo (pra|para) (o )?habito|quanto tempo (pra|para) virar|virar rotina|criar rotina|manter (a )?constancia|manter a constância|21 dias|habito aliment)/.test(norm)) {
      return `<b>Quanto tempo pra virar hábito?</b> ⏳<br><br>• O mito dos "21 dias" é falso. A ciência mostra que um hábito leva em média <b>~66 dias</b> — e varia de 18 a 250 dias conforme a pessoa e a complexidade.<br>• O segredo não é força de vontade, é <b>tornar fácil</b>: deixe a fruta lavada à vista, a marmita pronta, a garrafa de água na mesa.<br>• <b>Empilhe</b> o novo hábito num que já existe ("depois do café, tomo água").<br>• <b>Repetição > intensidade</b>: fazer pouco todo dia cria hábito; fazer muito e parar, não.<br>• Falhou um dia? Normal. <b>Nunca falhe duas vezes seguidas</b> — essa é a única regra.<br><br>Semanas tentando já é progresso. Siga que vira automático. 🔁`;
    }
    // lanche prático/portátil (levar na bolsa, sem geladeira)
    if (/(lanche (pra|para) levar|lanche portatil|lanche portátil|levar na bolsa|aguent. sem gelar|sem gelar|sem geladeira|lanche (pra|para) (a )?bolsa|lanche (pro|para o) trabalho|comer no trabalho)/.test(norm)) {
      return `<b>Lanches práticos que aguentam fora da geladeira:</b> 🎒<br><br>• <b>Castanhas/amêndoas/mix</b> (1 punhado): gordura boa e saciam bem.<br>• <b>Fruta resistente</b>: banana, maçã, pera, tangerina.<br>• <b>Barrinha de proteína</b> ou de frutas com poucos ingredientes.<br>• <b>Ovo cozido</b> (aguenta algumas horas), sanduíche de pão integral com pasta de amendoim.<br>• <b>Iogurte/whey em pó</b> (leva o pó e mistura com água na hora).<br>• <b>Biscoito de arroz + atum</b> em sachê, ou pipoca caseira.<br><br>Combo simples: 1 fruta + 1 punhado de castanhas = energia sem estragar. 🍎</br>`;
    }

    // motivação / desânimo / vontade de desistir da dieta
    if (/(motivac|motivaç|desanim|desist|sem vontade|perdi a vontade|nao aguento mais|não aguento mais|to cansad|tô cansad|recome[cç]|segunda (comeco|começo|come[cç]a|de novo|dnv)|comeco segunda|começo segunda|toda segunda|todo domingo|acabo com a dieta|estrag. (tudo|a dieta)|larguei a dieta|jogar tudo fora|nao consigo continuar|não consigo continuar|volto a ter animo|voltar a me cuidar|perdi a vontade de me cuidar|fracassad|desanimad)/.test(norm)) {
      return `<b>Quando bate o desânimo:</b> 💚<br><br>• Você não falhou — o método de "tudo ou nada" é que falha. Recomeçar toda segunda e largar na quarta é sinal de meta <b>rígida demais</b>, não de falta de força.<br>• Troque perfeição por <b>constância</b>: não existe "estragar a dieta". Um dia fora é 1 refeição em 21 na semana — segue no próximo prato, sem punição.<br>• <b>Metas de processo</b> (ex.: "proteína no almoço", "10 min de caminhada") são mais fáceis de bater que "emagrecer X kg" e alimentam a motivação.<br>• Motivação vem <b>depois</b> da ação, não antes: comece pequeno hoje e o ânimo aparece no caminho.<br>• Registre pequenas vitórias — o app te ajuda a ver o progresso que a balança esconde.<br><br>Se o desânimo é constante e pesado, vale conversar com um profissional — cuidar da cabeça é parte da saúde. 🤝`;
    }
    // leite faz mal para adulto?
    if (/(leite faz mal|leite (e|é) ruim|adulto (pode|nao pode|não pode) (tomar |beber )?leite|parar de tomar leite|leite prejudica)/.test(norm)) {
      return `<b>Leite faz mal para adulto? (mito comum)</b> 🥛<br><br>• Para quem <b>não tem intolerância à lactose nem alergia</b>, leite é alimento normal: proteína de alto valor, cálcio e vitaminas.<br>• A ciência <b>não</b> mostra que leite "faz mal" para adultos saudáveis — nem que causa muco, acne garantida ou inflamação para todos.<br>• <b>Intolerância à lactose</b> é real e comum: dá gases/estufamento. Solução: leite sem lactose, queijos curados ou iogurte (têm menos lactose).<br>• Não é obrigatório: se você não gosta ou passa mal, dá pra bater o cálcio com iogurte, queijo, tofu, sardinha e folhas verdes.<br><br>Resumo: sem intolerância, pode tomar. Com sintomas, use versões sem lactose. 🧀`;
    }
    // glutamato monossódico / MSG / miojo
    if (/(glutamato|monossodico|monossódico|\bmsg\b|realçador de sabor|realcador de sabor|ajinomoto|miojo faz mal)/.test(norm)) {
      return `<b>Glutamato monossódico (MSG) é veneno?</b> 🍜<br><br>• <b>Não.</b> Órgãos como FDA e OMS classificam o MSG como <b>seguro</b> nas quantidades usadas em alimentos. O tal "mal-estar do restaurante chinês" nunca se confirmou em estudos controlados.<br>• Glutamato é natural: existe em tomate, queijo parmesão, cogumelo e até no leite materno.<br>• O problema do <b>miojo</b> não é o MSG — é ser ultraprocessado, pobre em nutrientes e cheio de <b>sódio</b> (um pacote pode ter metade do sódio do dia).<br>• Comeu de vez em quando? Tudo bem. Só não faça base da alimentação e capriche na proteína/vegetais ao redor.<br><br>Foque em comida de verdade — o MSG isolado não é o vilão. 🧂`;
    }
    // microondas tira nutrientes?
    if (/(microondas|micro-ondas|microonda) (tira|destroi|destrói|mata|estraga|acaba com) /.test(norm) || /microondas faz mal|radiacao do microondas|radiação do microondas/.test(norm)) {
      return `<b>Micro-ondas tira os nutrientes? (mito)</b> ⚡<br><br>• <b>Todo</b> cozimento muda alguns nutrientes (calor + água). O micro-ondas costuma <b>preservar MAIS</b> vitaminas que ferver, porque cozinha <b>rápido e com pouca água</b>.<br>• Ele <b>não</b> deixa a comida "radioativa" nem cria substâncias tóxicas — as micro-ondas só agitam as moléculas de água e esquentam.<br>• Dica: aquecer legumes no vapor do micro-ondas retém bem a vitamina C.<br>• Cuidado real: use recipientes <b>próprios para micro-ondas</b> (evite plásticos não indicados).<br><br>Pode usar sem medo — é um dos métodos que menos "mata" nutriente. 🥦`;
    }
    // gordura saturada é vilã?
    if (/(gordura saturada|banha|manteiga faz mal|gordura (e|é) vil|colesterol da comida|ovo entope|entope a veia|gema faz mal)/.test(norm)) {
      return `<b>Gordura saturada é vilã? (o consenso mudou)</b> 🧈<br><br>• A visão atual é mais equilibrada: o vilão nº1 é o <b>excesso calórico e os ultraprocessados</b>, não a gordura saturada isolada.<br>• Gordura saturada de <b>comida de verdade</b> (ovo, carne, laticínios, coco) cabe numa dieta saudável em quantidade moderada.<br>• O <b>colesterol da comida</b> (ex.: gema) tem pouco impacto no colesterol do sangue para a maioria — pode comer ovo diariamente.<br>• Priorize gorduras <b>insaturadas</b> (azeite, castanhas, abacate, peixe) e evite <b>gordura trans</b> (a de verdade a evitar) e frituras de ultraprocessado.<br>• Quem tem colesterol alto/risco cardíaco deve individualizar com médico/nutri.<br><br>Equilíbrio > demonizar um nutriente. 🥑`;
    }
    // comer saudável com pouco dinheiro
    if (/(pouco dinheiro|sem grana|pouca grana|comida barata|comer barato|economizar na comida|20 reais|pouco din|apertad. de grana|orcamento apertado|orçamento apertado|gasto muito com ifood|gasto muito com comida|barato pra semana)/.test(norm)) {
      return `<b>Comer bem gastando pouco:</b> 💰<br><br>• <b>Base barata e nutritiva</b>: arroz, feijão, ovo, frango (peito/coxa), sardinha em lata, batata, banana, aveia, repolho, cenoura.<br>• <b>Ovo</b> é a proteína mais barata do mundo — coloque em tudo.<br>• Compre <b>frutas/legumes da estação</b> na feira no fim do dia (mais baratos).<br>• <b>Cozinhe em lote</b> e congele — evita ifood, que é o que come seu dinheiro.<br>• Fuja dos ultraprocessados: biscoito/refri/salgadinho são caros e não sustentam.<br>• Faça <b>lista</b> e não vá ao mercado com fome.<br><br>Com R$ arroz+feijão+ovo+frango+banana você monta a semana toda. 🍚`;
    }
    // festa / evento social / rodízio / aniversário
    if (/(festa|aniversario|aniversário|churrasco|rodizio|rodízio|evento|casamento|bar com amig|happy hour|vou sair|final de semana com amig|salgado e bolo)/.test(norm)) {
      return `<b>Sobreviver a festas/eventos sem culpa:</b> 🎉<br><br>• <b>Não vá com fome</b>: coma uma refeição normal (proteína + fibra) antes — você chega no controle, não devorando tudo.<br>• Dê a <b>primeira volta olhando</b> tudo, depois escolha o que <b>vale a pena</b> (não gaste calorias no salgado morno qualquer).<br>• <b>Prato montado</b> uma vez > beliscar de pé a noite inteira sem noção do quanto comeu.<br>• Álcool: alterne cada dose com <b>água</b> e lembre que ele é caloria líquida que solta o freio.<br>• Um evento não desfaz a semana. <b>Aproveite</b> sem drama e volte ao normal no próximo prato.<br><br>Equilíbrio social também é saúde. Curta! 🥂`;
    }
    // definir / recomposição / secar sem perder força
    if (/(definir|definic|definiç|secar sem perder|perder gordura sem perder|manter (a )?forca|manter a força|recomposicao|recomposição|trincar|ficar seco|ficar sarad)/.test(norm)) {
      return `<b>Definir (perder gordura mantendo músculo):</b> 🔪<br><br>• <b>Déficit moderado</b> (~300-500 kcal), não agressivo — corte grande derrete músculo junto.<br>• <b>Proteína alta</b>: 1,8-2,2g/kg é o que protege a massa magra no déficit.<br>• <b>Mantenha o treino de força pesado</b> — é o sinal de "preciso desse músculo". Só cardio + dieta = perde força.<br>• Perda saudável: ~0,5-1% do peso por semana. Mais rápido custa músculo.<br>• Durma bem e controle o estresse (cortisol atrapalha a definição abdominal).<br>• "Recomposição" (perder gordura e ganhar músculo ao mesmo tempo) é possível para iniciantes, quem voltou a treinar ou com % de gordura mais alto.<br><br>Paciência: definição é revelar o músculo que já existe. 💪`;
    }
    // comer a mesma coisa todo dia
    if (/(mesma coisa todo dia|comer sempre a mesma|repetir (a )?comida|enjoei|enjoad. de comer|de saco cheio de (comer|frango)|monotona|monótona|todo dia a mesma)/.test(norm)) {
      return `<b>Comer a mesma coisa todo dia é ruim?</b> 🍱<br><br>• Não faz o corpo "se acostumar e parar de emagrecer" — isso é mito. O que decide é o total de calorias.<br>• O risco real é <b>nutricional e psicológico</b>: cardápio muito repetido pode faltar variedade de nutrientes e te fazer <b>enjoar</b> e largar tudo.<br>• <b>Varie dentro da mesma estrutura</b>: troque a proteína (frango→ovo→peixe→carne), o carbo (arroz→batata→macarrão) e as cores dos vegetais.<br>• Repetir o <b>café da manhã</b> ou usar 3-4 opções em rodízio é ótimo pra praticidade — só não caia na monotonia total.<br><br>Simplicidade ajuda a manter; variedade ajuda a não desistir. Equilibre os dois. 🥗`;
    }
    // diabético tipo 2 / pressão alta — o que não pode / o que comer
    if (/(diabetic|diabético|tipo 2|nao pode comer|não pode comer|pressao alta|pressão alta|pressao 1|hipertens|baixar (a )?pressao|baixar a pressão)/.test(norm)) {
      return `<b>Diabetes e pressão alta — alimentação:</b> 🩺<br><br><b>Diabético (evitar/controlar):</b><br>• Açúcar e doces, refrigerante/suco, farinha branca (pão, massa), excesso de álcool.<br>• Prefira carbo integral, muita fibra e proteína — reduzem o pico de glicose.<br>• Fruta <b>pode</b> (com moderação e junto de proteína/fibra) — o vilão é o açúcar líquido.<br><br><b>Pressão alta:</b><br>• Corte o <b>sódio</b>: embutidos, enlatados, temperos prontos, ultraprocessados (não é só o saleiro).<br>• Aumente <b>potássio</b>: banana, feijão, folhas, batata.<br>• Padrão DASH (frutas, verduras, grãos integrais, laticínio magro) reduz pressão de verdade.<br><br>⚠️ Isto é orientação geral — diabetes e hipertensão precisam de acompanhamento <b>médico</b> e ajuste de remédios. Não pare medicação por conta própria. 💚`;
    }
